...
struct totalLabel: View {
    ...
    func getRecomended (nutrient: NutrientType) -> Float{
        ...
        if (user.gender == 0) {
            caloriesPerConsumir = (Float(user.weight) * 10)
            caloriesPerConsumir = caloriesPerConsumir + (Float(user.height) * 6.5)
            caloriesPerConsumir = caloriesPerConsumir - (Float(user.age) * 5) - 161
            print("dona")
        }else {
            caloriesPerConsumir = Float(user.weight) * 10
            caloriesPerConsumir += Float(user.height) * 6.25
            caloriesPerConsumir -= Float(user.age) * 5
            caloriesPerConsumir += 5
            print(caloriesPerConsumir)
        }
        
       
        
        if (user.sport == 0) {
            caloriesPerConsumir *= 1.2
        }else if (user.sport == 1){
            caloriesPerConsumir *= 1.375
        ...
        if (nutrient.name == "Proteïnes") {
            return Float(user.weight) * 1.5
                        
        }else if (nutrient.name == "Hidrats de carboni"){
            return caloriesPerConsumir / 4
        }else if (nutrient.name == "Lípids"){
            return caloriesPerConsumir * 30 / 100
            ...
            return caloriesPerConsumir / 1000
        }
    }
